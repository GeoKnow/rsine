  ...
  public URI getRemoteService(Resource resource) throws ServiceUnavailableException {
    try {
      ...
      if (responseHeaders.length == 1) {
        ...
        throw new ServiceUnavailableException("No rsine uri for resource " + resource.toString()
            + " found");
      }
    } catch (URISyntaxException e) {
      throw new ServiceUnavailableException("Resource " + resource.toString()
          + " ist not a valid URI");
    } catch (IOException e) {
      throw new ServiceUnavailableException("Error connecting to remote concept hosting service: "
          + resource.toString());
