...
public class MinTimePassedEvaluationPolicyTest {
  ...
  public void setUp() throws IOException, RepositoryException, RDFParseException,
    ...
    subscription.addQuery(new NotificationQuery(createQuery(), new ToStringBindingSetFormatter(),
        subscription));
  ...
  private String createQuery() {
    ...
    return Namespaces.SKOS_PREFIX + Namespaces.CS_PREFIX + Namespaces.DCTERMS_PREFIX + "SELECT * "
        + "WHERE {" + "?cs a cs:ChangeSet . " + "?cs cs:createdDate ?csdate . "
        + "?cs cs:removal ?removal . " + "?cs cs:addition ?addition . "
        + "?removal rdf:subject ?concept . " + "?addition rdf:subject ?concept . "
        + "?removal rdf:predicate skos:prefLabel . " + "?removal rdf:object ?oldLabel . "
        + "?addition rdf:predicate skos:prefLabel . " + "?addition rdf:object ?newLabel . "
        + "FILTER (?csdate > \"" + QueryEvaluator.QUERY_LAST_ISSUED
        + "\"^^<http://www.w3.org/2001/XMLSchema#dateTime>)" + "}";
  }

  protected void changePrefLabel() throws IOException {
    persistAndNotifyProvider.persistAndNotify(Helper.createChangeSetModel(
        "http://reegle.info/glossary/1111", "http://www.w3.org/2004/02/skos/core#prefLabel",
        new LiteralImpl("Ottakringer Helles", "en"), "http://reegle.info/glossary/1111",
        "http://www.w3.org/2004/02/skos/core#prefLabel", new LiteralImpl("Schremser Edelmärzen",
            "en"), ChangeTripleService.CHANGETYPE_UPDATE), true);
  }
  ...
  public void immediateNotificationOnFirstChange() throws IOException {
    ...
    Awaitility.await().atMost(2, TimeUnit.SECONDS).until(
        new NotificationDetector(timeMeasureNotifier));
  ...
  public void changeTooSoonForNotification() throws IOException {
    ...
    Awaitility.await().atMost(10, TimeUnit.SECONDS).until(
        new NotificationDetector(timeMeasureNotifier));
    Assert
        .assertTrue(timeMeasureNotifier.getMillisPassed() > IMMEDIATE_NOTIFICATION_THRESHOLD_MILLIS);
  }
