...
public class NotificationWithAuxiliaryTest {
  ...
  public void setUp() throws IOException, RepositoryException {
    ...
    registerSubscription(createBroaderRelationQuery(), new VelocityBindingSetFormatter(
        velocityTemplate));
  }
  ...
  private void registerSubscription(String query, BindingSetFormatter formatter) {
    ...
    notificationQuery.addAuxiliaryQueries(Arrays.asList(createConceptLabelQuery(),
        createOtherConceptLabelQuery()));
  ...
  private String createBroaderRelationQuery() {
    return Namespaces.SKOS_PREFIX + Namespaces.CS_PREFIX + Namespaces.DCTERMS_PREFIX
        + "SELECT ?concept ?otherConcept " + "WHERE {" + "?cs a cs:ChangeSet . "
        + "?cs cs:createdDate ?csdate . " + "?cs cs:addition ?addition . " +

        "?addition rdf:subject ?concept . " + "?addition rdf:predicate skos:broader . "
  ...
  private String createConceptLabelQuery() {
    return Namespaces.SKOS_PREFIX
        + "SELECT ?conceptLabel WHERE {?concept skos:prefLabel ?conceptLabel}";
  }

  private String createOtherConceptLabelQuery() {
    return Namespaces.SKOS_PREFIX
        + "SELECT ?otherConceptLabel WHERE {?otherConcept skos:prefLabel ?otherConceptLabel}";
  }
  ...
  public void prefLabelsInMessage() throws IOException {
    ...
    Assert.assertTrue(messageConcatenatingNotifier.message
        .contains(otherConceptUriLabel.getLabel()));
  }

  private void persistChangeSet() throws IOException {
    Model changeSet = new ChangeSetCreator().assembleChangeset(new StatementImpl(otherConceptUri,
        SKOS.BROADER, conceptUri), null, ChangeTripleService.CHANGETYPE_ADD);
